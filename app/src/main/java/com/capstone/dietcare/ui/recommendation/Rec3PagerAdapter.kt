...
class Rec3PagerAdapter(private val recItem : List<DataItem>): RecyclerView.Adapter<Rec3PagerAdapter.Rec3ViewHolder>() {
    class Rec3ViewHolder (private val binding: ItemRecommendationRecipeBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind (data : DataItem){
            ...
            binding.btnEat.setOnClickListener {
                ...
                val recIntent = Intent(itemView.context, Recommendation4Activity::class.java)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Rec3ViewHolder {
        val binding = ItemRecommendationRecipeBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return Rec3ViewHolder(binding)
    }

    override fun getItemCount(): Int = recItem.size

    override fun onBindViewHolder(holder: Rec3ViewHolder, position: Int) {
