...
import com.capstone.dietcare.databinding.ActivityRecommendation4Binding
import com.capstone.dietcare.ui.history.HistoryActivity
import com.google.android.material.snackbar.Snackbar
import com.google.gson.JsonObject

class Recommendation4Activity : AppCompatActivity() {

    private lateinit var binding: ActivityRecommendation4Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityRecommendation4Binding.inflate(layoutInflater)
        setContentView(binding.root)
        supportActionBar?.title = "Choose Your First Snack"
    ...
    private fun getRecIntent() {
        ...
        if(recom !=null){
            recViewModel.updateMeal3(recom)
        }
    }
    ...
    private fun getRecommendation(){
        recViewModel.getLatestMeal().observe(this){ nutrition->
            if (nutrition != null){
                if (nutrition.m3cal != null){
                    binding.tvCalLeft.text = "Remaining calorie needs : ${nutrition.calor?.toInt()?.minus(nutrition.m1cal!!.toInt())?.minus(nutrition.m2cal!!.toInt())?.minus(nutrition.m3cal!!.toInt())} kcal"
                }
                if (nutrition.meal == 3){
    ...
    private fun setPager(it: List<DataItem>?) {
        ...
        pager.adapter = Rec4PagerAdapter(it!!)
