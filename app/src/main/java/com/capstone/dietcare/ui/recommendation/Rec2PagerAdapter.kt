...
class Rec2PagerAdapter(private val recItem : List<DataItem>): RecyclerView.Adapter<Rec2PagerAdapter.Rec2ViewHolder>() {
    class Rec2ViewHolder (private val binding: ItemRecommendationRecipeBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind (data : DataItem){
            ...
            binding.btnEat.setOnClickListener {
                ...
                val recIntent = Intent(itemView.context, Recommendation3Activity::class.java)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Rec2ViewHolder {
        val binding = ItemRecommendationRecipeBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return Rec2ViewHolder(binding)
    }

    override fun getItemCount()= recItem.size

    override fun onBindViewHolder(holder: Rec2ViewHolder, position: Int) {
