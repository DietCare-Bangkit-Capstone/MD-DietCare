...
class Rec4PagerAdapter(private val recItem : List<DataItem>): RecyclerView.Adapter<Rec4PagerAdapter.Rec4ViewHolder>() {
    class Rec4ViewHolder (private val binding: ItemRecommendationRecipeBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind (data : DataItem){
            ...
            binding.btnEat.setOnClickListener {
                ...
                val recIntent = Intent(itemView.context, Recommendation5Activity::class.java)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Rec4ViewHolder {
        val binding = ItemRecommendationRecipeBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return Rec4ViewHolder(binding)
    }

    override fun getItemCount()=recItem.size

    override fun onBindViewHolder(holder: Rec4ViewHolder, position: Int) {
