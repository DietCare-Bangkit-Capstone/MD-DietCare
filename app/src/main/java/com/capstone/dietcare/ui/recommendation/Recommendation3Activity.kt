...
import com.capstone.dietcare.databinding.ActivityRecommendation3Binding
import com.google.android.material.snackbar.Snackbar
import com.google.gson.JsonObject

class Recommendation3Activity : AppCompatActivity() {

    private lateinit var binding: ActivityRecommendation3Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityRecommendation3Binding.inflate(layoutInflater)
        setContentView(binding.root)
        supportActionBar?.title = "Choose Your Dinner"
    ...
    private fun getRecIntent() {
        ...
        if(recom !=null){
            recViewModel.updateMeal2(recom)
        }
    }
    ...
    private fun getRecommendation(){
        recViewModel.getLatestMeal().observe(this){ nutrition->
            if (nutrition != null){
                if (nutrition.m2cal != null){
                    binding.tvCalLeft.text = "Remaining calorie needs : ${nutrition.calor?.toInt()?.minus(nutrition.m1cal!!.toInt())?.minus(nutrition.m2cal!!.toInt())} kcal"
    ...
    private fun setPager(it: List<DataItem>?) {
        ...
        pager.adapter = Rec3PagerAdapter(it!!)
